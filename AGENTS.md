...
data/clinical/normalized/labs.json ──────────────── local lab history ────────────────────┤
...
| Path | What |
|---|---|
| `packages/core` | Everything data: Drizzle schema + migrations (`drizzle/`), **metric registry** (`src/registry.ts`), wire protocol (`src/payloads.ts`, Zod — mirrored by `ios/Baymax/Payloads.swift`), clinical/lab normalization (`src/clinical.ts`, `src/labs.ts`), ingest, sleep/trend queries, `HealthClient` SDK (`src/client.ts`), fixture generator + test helpers (`test/fixtures.ts`) |
| `apps/server` | Hono server. Ingest: `POST /v1/ingest/{samples,workouts}`. App reads: `GET /v1/{today,overview,nutrition,labs,labs/trend,log/template,backfill/bodyweight,backfill/clinical}`. App writes: `POST /v1/log/workout` (appends weights.json + re-imports). `GET /v1/ping` |
...
| `ios/` | SwiftUI companion app (iOS 26 Liquid Glass): Health sync, home tiles fed by `/v1/today` + `/v1/overview`, in-gym workout logging with offline queue (Log tab), and a nutrition mirror that writes intake days into Apple Health. Project generated by XcodeGen from `ios/project.yml` |
...
| `docs/clinical.md` | Local-only clinical archive, provider PDF/CSV parsers, general lab history, reviewed scan values, normalized HealthKit payload, and idempotent iPhone backfill |
...
bun run clinical:import [--check] # parse/validate local clinical records + general labs
...
CLI: `health overview | nutrition | lifts --exercise bench | labs [--marker apob] |
lab-trend --marker apob | status | sources | metrics | sleep --days 7 |
workouts --days 30 | samples --metric heart_rate --days 2 | trend --metric steps
--days 90`, all with `--json`.
...
- **Gym session** → the app's Log tab (prefills last session; queues offline at
...
(adaptive calorie/protein targets for the body-weight goal), **`health_labs`**
(full local panels), **`health_lab_trend`** (one-marker history), `health_status`,
