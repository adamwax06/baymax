...
| Path | What |
|---|---|
| `packages/core` | Everything data: Drizzle schema + migrations (`drizzle/`), **metric registry** (`src/registry.ts`), wire protocol (`src/payloads.ts`, Zod — mirrored by `ios/Baymax/Payloads.swift`), clinical normalization (`src/clinical.ts`), ingest, sleep/trend queries, `HealthClient` SDK (`src/client.ts`), fixture generator + test helpers (`test/fixtures.ts`) |
| `apps/server` | Hono server. Ingest: `POST /v1/ingest/{samples,workouts}`. App reads: `GET /v1/{today,overview,nutrition,log/template,backfill/bodyweight,backfill/clinical}`. App writes: `POST /v1/log/workout` (appends weights.json + re-imports). `GET /v1/ping` |
...
| `docs/clinical.md` | Local-only clinical archive, Lehigh PDF parser, reviewed scan values, normalized HealthKit payload, and idempotent iPhone backfill |
...
bun run clinical:import [--check] # parse/validate local Lehigh records for HealthKit backfill
